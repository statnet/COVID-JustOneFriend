...
- One important feature of a network that helps us to understand how far something might spread on it is the size of the **"largest connected component"**. That refers to the largest set of households who are all reachable to one another along any possible path. In this case, every household can be reached from every other household along at least one path---indeed, along many different ones---so the largest connected component includes **`r largcomp.precov`, or `r round(100*largcomp.precov/n, 0)`%, of the households**.
